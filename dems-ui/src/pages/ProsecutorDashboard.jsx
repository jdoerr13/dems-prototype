...
export default function ProsecutorDashboard() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-8 space-y-8">
      ...
      <p className="text-gray-600">Approvals, discovery, AI alerts, and co-defendants.</p>

      {/* Stat Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard title="Pending Approvals" value={pending.length} color="bg-yellow-600" path="/prosecutor" />
        <StatCard title="Accepted Cases" value={counts.acceptedCases} color="bg-green-600" path="/prosecutor" />
        <StatCard title="Defense Assigned" value={counts.defenseAssigned} color="bg-purple-600" path="/prosecutor" />
        <StatCard title="Co-Defendants" value="-" color="bg-gray-500" path="/codefendants" />
        <StatCard title="Defense Delegation" value="-" color="bg-gray-600" path="/delegation" />
        <StatCard title="Redaction Tools" value="-" color="bg-pink-600" path="/redaction" />
        <StatCard title="Analytics" value="-" color="bg-indigo-600" path="/analytics" />
      </div>
      ...
      {/* AI Flags */}
      <div className="bg-white rounded-xl shadow p-6">
        <h2 className="text-lg font-semibold mb-3">AI Flagged Items</h2>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={charts.aiTags.filter(t => ["weapon","firearm","drug paraphernalia"].includes(t.name))}>
