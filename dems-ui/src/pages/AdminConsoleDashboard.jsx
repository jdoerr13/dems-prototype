  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Cases" value={counts.totalCases} color="bg-blue-600" path="/admin" />
        <StatCard title="Evidence Items" value={counts.evidenceItems} color="bg-green-600" path="/admin" />
        <StatCard title="Audit Logs" value={counts.auditsCount} color="bg-red-600" path="/advanced-audit" />
        <StatCard title="User Management" value={counts.notificationsCount} color="bg-indigo-600" path="/users" />
        <StatCard title="Advanced Audit" value="-" color="bg-teal-600" path="/advanced-audit" />
        <StatCard title="Transparency Logs" value="-" color="bg-blue-800" path="/transparency-internal" />
        <StatCard title="Analytics Dashboard" value="-" color="bg-indigo-600" path="/analytics" />
        <StatCard title="Cross-Agency Governance" value="-" color="bg-blue-700" path="/cross-agency" />
        <StatCard title="Archival" value="-" color="bg-gray-800" path="/archival" />
      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
            <Pie data={charts.casesByAgency} dataKey="value" nameKey="name" outerRadius={100} label>
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <ul className="divide-y text-sm">
          {notifications.map(n => (
            <li key={n.id} className="py-2">{n.message}</li>
          ))}
        </ul>
      </div>
    </section>
